<template>
  <div class="cart_list_item">
    <cart-check-button @click.native="checkClick" :isChecked="item.checked" ref="checkButtonRef" />
    <div class="item_img">
...
export default {
  ...
  methods: {
    checkClick() {
      this.$store.state.CartList.forEach(listItem => {
        console.log(listItem);
        if (listItem.iid === this.item.iid) listItem.checked = !listItem.checked
      })
    },
    //点击回到该商品详情界面
    infoClick(item) {
      this.$router.push({ path: '/detail', query: { id: item.iid } })
    }
  },
  components: {
    CartCheckButton
